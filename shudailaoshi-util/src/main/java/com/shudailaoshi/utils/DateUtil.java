...
public class DateUtil {
	...
	public static List<Long> getBespeakTimeList(long beginNbkTime) {
		// 预约时间模板
		List<Long> time = new ArrayList<Long>();
		Calendar bespeakTime = Calendar.getInstance();
		bespeakTime.setTimeInMillis(beginNbkTime);
		bespeakTime.set(Calendar.SECOND, 0);// 秒
		for (int i = 0; i < MINUTE_ARRAY.length; i++) {
			bespeakTime.set(Calendar.HOUR_OF_DAY, Integer.valueOf(MINUTE_ARRAY[i].split(":")[0]));// 时
			bespeakTime.set(Calendar.MINUTE, Integer.valueOf(MINUTE_ARRAY[i].split(":")[1]));// 分
			time.add(bespeakTime.getTimeInMillis());
		}
		return time;
	}
	...
	public static Date timeToDate(final long time, String pattern) {
		return stringToDate(timeToString(time, pattern), pattern);
	}
	 ...
	 * 获得前？分钟毫秒数
	 */
	public static long getBeforeMinute(final Date date, int minute) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
	...
	public static void main(String[] args) {
		System.out.println((DateUtil.getTime() - 1513047315000L) / (1000 * 60));
		System.err
				.println(DateUtil.timeToString(
						DateUtil.getBeforeMinute(DateUtil.stringToDate(
								DateUtil.timeToString(DateUtil.getTime(), DateUtil.TIME), DateUtil.TIME), 20),
						DateUtil.TIME));
	}
